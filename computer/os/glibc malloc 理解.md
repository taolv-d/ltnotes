...
update: 2026-08-27
---
本文整理自博客 Understanding glibc malloc^[https://sploitfun.wordpress.com/2015/02/10/understanding-glibc-malloc/comment-page-1/] 及其中文翻译^[https://ltfa1l.top/2023/09/23/system/Heap/understanding\_glibc\_malloc/] 。
博客基于Linux下glibc 2.23 的 ptmalloc2 分配器（即 `malloc/free` 的实现）介绍：
* 内存分配的设计哲学（效率 vs. 安全性）
...
* 程序起来时，就已经有一个chunk 位于主arena，此时 top chunk 记录了内存最顶端的位置。后续 malloc 则从 top chunk 分割。此时所有 bins 都是空的
* 如果第一次请求内存超过 top chunk 大小，但是不足以出发 mmap，此时使用 brk 系统调用分配
* 否则使用 mmap 加载（需要虚拟内存到物理内存的映射）
* 对于非主线程：
...
* Chunk 中记录了上一个chunk的size即prev_size，理论上可以反向遍历。但实际不行，原因是只有上一个chuak为free时才成立，即全都free才能反向遍历
## Bins
...
```
### 什么时候调用brk, 什么时候mmap
1. `brk` 主要在主arena的小块内存申请（< mmap_threshold）且top chunk不足时调用。
2. mmap在三种情况下调用：
	1. thread arena的所有堆扩展
	2. 任何arena的大内存分配（≥ mmap_threshold）
