...
| 指标 | 说明 | 示例 |
| --- | --- | --- |
| 量子效率 | 特定波长下单位时间内产生的平均光电子数与入射光子数之比。 |  |
| 满阱容量 | 一个像素的势阱最多能够容纳多少个光生电子。消费类的 sensor 一般以 2000~4000 较为常见，此值越大则 sensor 的动态性能越好。 |  |
| 噪声 | 在处理过程中设备自行产生的信号，这些信号与输入信号无关。 |  |
| Black level | Sensor 无光时输出的信号强度。 |  |
| 信噪比 | 信噪比是一个电子设备或者电子系统中信号与噪声的比例。信噪比很低时，单纯的放大操作并不增加任何有用的信息，无助于改善图像质量。 | ![](<attachments/cmos-image-sensor-cis-image-23.png>) ![](<attachments/cmos-image-sensor-cis-image-24.png>) |
| 动态范围 DR | 动态范围表示图像中所包含的从“最暗”至“最亮”的取值范围。根据 ISO15739 的定义，“最亮”指能够使输出编码值达到特定“饱和值”的亮度；“最暗”指图像信噪比下降至 1.0 时的亮度。<br>人眼：80~100 dB<br>消费级 sensor：50~70 dB<br>车规级 sensor：120 dB（甚至不需要调节曝光） | ![](<attachments/cmos-image-sensor-cis-image-25.png>) ![](<attachments/cmos-image-sensor-cis-image-30.png>) |
| 串扰 | 光子可以在硅片中穿透一定距离，从而影响其他像素的信号。 | ![](<attachments/cmos-image-sensor-cis-G0o7bMCIIocerfxhAU0cl7Blnpe.png>) |
| 灵敏度 | CMOS sensor 对入射光功率的响应能力。常用定义是在 1 μm^2 单位像素面积上，标准曝光条件下（1 Lux 照度，F5.6 光圈），在 1 s 时间内积累的光子数能激励出多少 mV 的输出电压。<br>- 在图像噪声水平接近的情况下，灵敏度高的 sensor 图像亮度更高、细节更丰富。<br>- 在图像整体亮度接近的情况下，灵敏度高的 sensor 噪声水平更低，图像画质更细腻。 | ![](<attachments/cmos-image-sensor-cis-AZZybRFdcovUqHxya8JcLyjInxd.jpg>) |
| 填充系数 | 接收光的面积 / 总面积。微透镜可以将填充系数提高到 90%。 | ![](<attachments/cmos-image-sensor-cis-image-31.png>) ![](<attachments/cmos-image-sensor-cis-image-32.png>) |
| 像素尺寸 | 大的像素通常可以容纳更多电子，因此可以表示更大的信号变化范围，这个指标称为 sensor 的动态范围。在极低照度下，大像素更容易捕获到少量光子，也就是低照度性能更好。 | ![](<attachments/cmos-image-sensor-cis-image-33.png>) |
| 主光线角 CRA | 主光线角（Chief Ray Angle, CRA）是衡量 sensor 收集入射光能量的主要参考指标。 | ![](<attachments/cmos-image-sensor-cis-image-34.png>) |
