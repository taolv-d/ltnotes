...
RCD 全名是 Ratio Corrected Demosaicing。
### RCD 和 AMaZE 的核心差异
...
也就是用两个方向组的结构能量来决定融合权重。

**green 插值不同**

AMaZE 先用四方向 `ratio + Hamilton-Adams` 得到 `vcd/hcd`，再通过 `hvwt` 融合，并且后面还有 Nyquist refinement 和 `rbint` correction。
...
```text
nyqutest > 0 -> nyquist mask -> area interpolation -> green 二阶差分细化
```

RCD 没有这么重的 Nyquist 专门分支，主要靠方向判别图、低通比例修正和色差传播保持稳定。
...
对角估 missing R/B
构造 rbint
必要时用 rbint 反向修正 green
再做 fancy chroma interpolation
```
...
### 5.3 Step 1：建立水平/垂直方向图 VH_Dir

对比AMaZE，这里也是计算水平垂直梯度的权重，但简单的多

RCD 先计算垂直和水平的高通响应平方。垂直方向类似（水平方向同理，不写了）：
...
### 5.5 Step 3：在 R/B 点插 green

这是 RCD 的核心 green 恢复步骤。

先计算四个 cardinal gradients， 也是判断这个方向是否平滑（色差越平滑越越可靠）：
```text
