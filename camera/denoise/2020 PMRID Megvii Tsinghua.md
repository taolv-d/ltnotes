...
create: 2026-05-13
publish: 2020
url: https://arxiv.org/abs/2010.06935
update: 2026-07-20
---
论文： [**Practical Deep Raw Image Denoising** on Mobile Devices](https://arxiv.org/abs/2010.06935)

本文的目的是能否用一个轻量网络进行 raw 域降噪任务，这样可以用于手机等低算力平台。
**本文的思想**：能否将图像中随亮度变化的噪声变成更稳定的噪声分布，这样就可以用一个简单的网络来进行降噪。
除了 k-sigma 变换外，文章还介绍了如何标定噪声参数，这对构造数据提供了理论指导。

# k-sigma 变换降低降噪难度

- **直接降噪的难点**：ISO 100 和 ISO 6400 的噪声完全不是一个量级。如果直接喂给一个网络，网络会很困惑，难以收敛（或者需要一个大网络才能处理）。
...
- 对比VST 方差 ~= 1
- VST 逆变换会放大误差，可能引入artifartifacts
...
论文中提到的噪声模型标定 现在已经比较常见，很多商用ISP已经支持。
首先是噪声模型，本文使用的是最常见的噪声模型（高斯噪声+泊松噪声）。

基于**泊松-高斯噪声模型**来描述RAW图像的噪声：
- **散粒噪声**：与进光量有关，光越强，噪声波动越大。
- **读出差分噪声**：与传感器电路有关，和进光量无关，但和ISO（增益）强相关。

**测量方法**： 对着灰度卡连拍64张，算出每个像素位置的**均值**（当作真值）和**方差**（当作噪声强度）。通过线性回归，就能拟合出不同ISO下的噪声参数 $k$ 和 $\sigma^2$。参考下图。

![[attachments/Pasted image 20260720232521.png]]
...
![[attachments/Pasted image 20260720232441.png|548]]
参考上图，噪声模型如下：
$$ x = g(\alpha u + n_d) + n_r $$
其中：
*   $g$：模拟增益（与ISO相关）
