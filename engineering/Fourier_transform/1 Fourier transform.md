...
![[attachments/Pasted image 20260814212951.png|419]]

**吉布斯现象**：当一个**不连续信号**（如方波、锯齿波）用有限项傅里叶级数逼近时，在间断点附近会出现**持续的过冲和振荡**。（想象下：有限个连续光滑的函数通过叠加组合的 没办法得到一个跳跃的点）

- 过冲幅度约为跳变幅度的 **9%**
- 无论取多少项，过冲不会消失，只是被压缩到更窄的时间区间内
- 振荡频率随着项数增加而提高，但过冲幅度恒定

**吉布斯现象不是“误差”，而是“截断”的固有特性。**
## 数学根源

**傅里叶级数的部分和**
设周期信号为 $x(t)$，周期为$T_0$，基频 $\omega_0 = 2\pi/T_0$。取前 $N$ 次谐波的部分和：
$$
x_N(t) = \sum_{k=-N}^{N} c_k e^{jk\omega_0 t}
$$
我们的目标是：当 $N \to \infty$ 时，$x_N(t) \to x(t)$。

**利用卷积定理**
傅里叶级数的部分和等价于原信号与 **Dirichlet 核**（sinc 函数的离散版本） 的卷积（这一步跟信号重建那里，用sinc 函数与采样点卷积是一致的）：
$$
x_N(t) = (x * D_N)(t) = \int_{-\infty}^{\infty} x(\tau) D_N(t - \tau) d\tau
$$
再往下的推导比较复杂了（需要计算跳变点的极限等），暂时不展开，结论就是跳变点的过冲不为0，约为9%，跟具体函数的形式无关。

## 如何减轻吉布斯现象

**使用窗函数（不是矩形窗）**

不要在时域直接截断，而是用**平滑的窗函数**乘上理想系数：$h[n] = h_d[n] \cdot w[n]$
常用窗函数及其旁瓣衰减：
...
| 布莱克曼窗 | -57 dB | 更宽 |
| 凯泽窗（β=8） | -63 dB | 可调 |
窗函数在边界处平滑过渡到零，抑制了 Dirichlet 核的旁瓣幅度，从而减轻了过冲和振荡。
...
**8点FFT 分解示意图**
![[attachments/Pasted image 20260812234547.png|527]]

# FIR IIR 滤波器

**滤波器本质上是“频率选择器”**
傅里叶变换的核心思想就是：**任何信号都可以分解为不同频率的正弦波之和。** 因此，要设计一个滤波器，首先就要在频域去思考：我们需要保留哪些频率，抑制哪些频率。
## FIR 滤波器

**FIR** 全称 **Finite Impulse Response**（有限冲激响应）。
### 时域
它的输入输出关系是一个**卷积和**：
$$
y[n] = \sum_{k=0}^{M-1} h[k] \cdot x[n-k]
$$
其中：$h[k]$  是滤波器的**冲激响应**，长度为 $M$（有限长）。也就是当前的输出  $y[n]$ 只依赖于**最近的 $M$ 个输入值**（ $x[n], x[n-1], \dots, x[n-M+1]$），不依赖过去的输出。这就是“有限冲激响应”的含义——冲激响应在 $M$ 个点之后完全归零。
### 频域
直接用卷积定理：
$$
Y(e^{j\omega}) = X(e^{j\omega}) \cdot H(e^{j\omega})
$$
所以，**设计 FIR 滤波器，本质上就是设计一组系数 $h[k]$，使得 $H(e^{j\omega})$ 在频域上呈现出你想要的形状**（低通、高通、带通等）。

### 特性
如果滤波器系数满足**对称性**（偶对称或奇对称），则滤波器具有**严格的线性相位**：
**偶对称情况（最常用）**：
$$
h[k] = h[M-1-k], \quad k = 0, 1, \dots, M-1
$$
频率响应：
$$
H(e^{j\omega}) = A(\omega) \cdot e^{-j\omega (M-1)/2}
$$
其中：
- $A(\omega)$ 是**实数的幅度函数**（可正可负）
- 相位项 $e^{-j\omega (M-1)/2}$ 是**纯线性相位**
**优点**
- 所有频率分量通过滤波器后，**延迟完全相同**（群延迟为常数 $(M-1)/2$ 个采样点）。
- 信号波形经过滤波器后**形状不变**，只是整体平移。避免相位失真造成的信号畸变。
**奇对称情况**
$$
h[k] = -h[M-1-k]
$$
相位响应包含一个固定的 $\pi/2$ 相移，同样具有线性相位特性，但频率响应在 $\omega=0$ 处必为零。

## IIR 滤波器

**IIR** 全称 **Infinite Impulse Response**（无限冲激响应）。
它的输入输出关系是一个**差分方程**：
$$
y[n]=\sum_{k=0}^{M} b_k x[n-k] - \sum_{k=0}^{N} a_k y[n-k]
$$

其中：$b_k$前向系数（与输入相关）,$a_k$反馈系数（与过去的输出相关）

IIR 的输出不仅依赖于当前和过去的输入，还依赖于**过去的输出**。这种递归结构使得冲激响应无限长，永远不会完全归零。

IIR 的频率响应要用 Z 变换，这里直接给出结果：
$$
H(e^{j\omega}) = \frac{\sum_{k=0}^{M} b_k e^{-j\omega k}}{1 + \sum_{k=1}^{N} a_k e^{-j\omega k}}
$$
其中
- 分子的结构与 FIR 滤波器完全相同（前向路径）。
- 分母的存在使得 IIR 能够用**低阶数**实现陡峭的频率选择特性——这是 IIR 相比 FIR 最核心的优势。（FIR 频率响应是多项式，想要陡峭的边缘需要很多（几十上百）个正弦波逼近方波。IIR 让分母接近0就可以实现，只需要几阶）
## IIR 与 FIR 对比

| 特性            | FIR             | IIR             |
| :------------ | :-------------- | :-------------- |
| **冲激响应**      | 有限长（M 个点后归零）    | 无限长（递归反馈）       |
| **系统函数**      | 只有零点（全零点）       | 零点和极点（有理函数）     |
| **稳定性**       | **无条件稳定**       | 需检查极点位置         |
| **相位特性**      | 可严格线性相位         | 非线性相位（群延迟通常不平坦） |
| **实现相同指标的阶数** | 通常较高（几十到几百阶）    | 通常较低（几到十几阶）     |
| **计算量**       | 较大（乘法数 ≈ M）     | 较小（乘法数 ≈ M+N）   |
| **设计自由度**     | 任意指定频响          | 受模拟原型限制         |
| **有限字长效应**    | 较不敏感            | 较敏感（极点可能漂移）     |
| **设计方法**      | 窗函数法、频率采样法、等波纹法 | 双线性变换、冲激响应不变法   |

FIR IIR 滤波器还有很多讲究，例如如何设计滤波器。这里先不深入研究了
